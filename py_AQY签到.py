...
class IQY_sign:
    ...
    *奖励：签7天奖1天，14天奖2天，28天奖7天；日常任务；随机成长值
    ...
    def draw(self, i):
        ...
        res = requests.get(url, params=params1, headers=headers, verify=False)
        # 提取抽奖url前缀
        # print(res.text)
        for i, card in enumerate(res.json()["cards"]):
            for j, block in enumerate(card["blocks"]):
                url = block.get("actions", {}).get(
                    "click_event", {}).get("data", {}).get("url")
                if url and url.startswith("http://iface2.iqiyi.com/"):
                    ...
                    res = requests.get(url, params=dict(
                        params1, **params2), headers=headers)
                    break
...
if __name__ == '__main__':
    P00001 = os.environ.get('AQY_P00001')
    # 签到
    obj = IQY_sign(P00001)
    msg1 = obj.sign()
    # 抽奖
